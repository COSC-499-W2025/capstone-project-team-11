...
| :---- | :---- | :---- | :---- | :---- |
| The system must be able to display information based on file creation dates.  | Users can choose to view artifacts within a certain date range to filter their preferred data.  **Steps:** Collect file metadata Store and index the dates Provide filtering for data Process the data Display the data to user **Possible difficulties:**  Inconsistent file creation Large dataset performance User privacy  | **Test Case 1:** Verify system can display all artifacts sorted by file creation date **Preconditions:** System has completed a scan and stored artifacts with creation dates **Test Steps:**          1\. Open dashboard 2\. 2\. Select “Sort by Creation Date (Ascending)” **Expected result:** Work displayed from oldest to newest based on its creation date **Test Case 2:** Verify filtering by date range returns correct results. **Preconditions**: System has work spanning multiple months.  **Test Steps:**              1\. Open dashboard  2\. Select date filter 3\. Apply filter **Expected result:** Only artifacts created between the date range are displayed.    | Daniel | Medium  |
| The system must be able to categorize files by type (e.g., documents, code, media). | This feature allows the system to categorize work into file type categories such as code, documents, media, and design files.  **Steps:**  Scan files Identify file type Store category in database Display categorised results.  **Possible difficulties**  Ambiguous file types Custom formats Scalabality  | **Test Case 1:** Verify code files are categorised correctly. **Preconditions:** System scanned folders with .py, .java, .cpp files. **Test Steps:**             1\. Open dashboard 2\. Filter by “code” **Expected Result:** Only code files are displayed. **Test Case 2:** Verify document files are categorised correctly. **Preconditions:** System scanned folders with .docx, .pdf, .txt. **Test Steps:**             1\. Open dashboard  2\. Filter by “document” **Expected Result:** Only document files are displayed. **Test Case 3:** Verify media files are categorised correctly.  **Preconditions:** System scanned folders with .jpg, .png, and .mp4. **Test Steps:**             1\. Open dashboard  2\. Filter by “media” **Expected Result:** Only media files are displayed.  | Travis | Medium |
| The system must present scan results visually in clear, human-readable formats such as PDF, HTML (printable webpage), PNG, or JPG. | The user can select between several output formats such as PDF, JPG, PNG, or HTML.  **Steps:** Generate report data Render visuals such as charts, tables, summary counts, and key metrics. Export to desired format. Present to user **Possible difficulties:** Formatting consistency  Chart rendering for exports Cross-platform file generation | **Test Case 1:** Verify export type works with summary data. **Preconditions:** Scan completed with results.  **Test Steps:**             1\. Go to dashboard  2\. Click export as desired format        3\. Save file              4\. Open file **Expected Result:** Desired file format contains correctly formatted summary, charts, and tables. **Test Case 2:** Verify export respects filters. **Preconditions:** User applied category/dates filter. **Test Steps:**             1\. Apply filter         2\. Export report       **Expected Result:** Report only contains filtered data.  | Tyler | Hard |
| The system must be able to scan user-selected directories to locate relevant work artifacts. | The system must be able to scan user-selected directories to locate relevant work artifacts. **Steps:** User selects directories to include. System validates path accessibility. System traverses the directories. Metadata (name, size, type, creation date) is collected. Results are stored and displayed. **Possible difficulties:** Handling inaccessible or permission-restricted folders Large dataset traversal performance Ensuring the scan is non-blocking for the UI | **Test Case 1:** Verify system scans a valid directory. **Preconditions:** User has selected a valid directory with files. **Test Steps:** Open application. Select directory containing files. Start scan. **Expected Result:** All files in the directory are scanned and displayed with metadata. **Test Case 2:** Verify system handles empty directory. **Preconditions:** User selects an empty directory. **Test Steps:** Open application. Select empty directory. Start scan. **Expected Result:** System completes scan with “no files found” message, no crash. **Test Case 3:** Verify invalid or inaccessible directory. **Preconditions:** User selects an invalid or restricted path. **Test Steps:** Open application. Select directory without access. Start scan. **Expected Result:** System shows error notification, does not crash, and continues to function. | Jaxson | Medium |
